    ...
    return (
        `<article class="bg-white dark:bg-slate-800 shadow-xl shadow-slate-200 dark:shadow-slate-800 rounded-lg">
            ...
            <div class="grid gap-1">
                ...
                ${e.location ? `<p class="text-slate-400 text-sm">${e.location}</p>` : ''}
                </h2>
                ${e.description ? `
                <div class="grid gap-1">
            ...
            <a href="${e.link}" class="bg-indigo-500 rounded-md px-4 py-2 text-indigo-50 shadow-2xl shadow-indigo-200 dark:shadow-none text-center font-bold hover:shadow-none ring ring-offset-0 ring-indigo-500 focus:outline-none focus:ring-offset-2">View Event</a>
            </div>
        </article>`
    )
